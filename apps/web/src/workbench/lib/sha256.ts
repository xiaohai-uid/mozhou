/**
 * 客户端 SHA-256 计算（与后端 createHash('sha256') UTF-8 十六进制一致）。
 * 优先使用标准 Web Crypto API（浏览器与现代 Node 原生支持）；
 * 兜底提供标准轻量纯 JS 实现，不依赖 node:crypto。
 */
export async function computeSha256Hex(text: string): Promise<string> {
  ...
  return syncSha256Hex(text)
}

/** 纯 JS 同步 SHA-256（供无 Web Crypto 环境兜底）。 */
export function syncSha256Hex(text: string): string {
  const K = [
