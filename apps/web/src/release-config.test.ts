...
describe('release hardening · distribution configuration', () => {
  ...
  it('documents the exact release artifact names generated by the build script', () => {
    const rootPackage = JSON.parse(read('package.json')) as { version: string }
    const readme = read('README.md')

    for (const suffix of ['local-runtime', 'web-dist', 'docker']) {
      expect(readme).toContain(`mozhou-v${rootPackage.version}-${suffix}.tar.gz`)
      expect(readme).not.toContain(`mozhou-${rootPackage.version}-${suffix}.tar.gz`)
    }
  })
