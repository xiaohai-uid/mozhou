...
export const CAPABILITY_DETAILS: Record<string, CapabilityDetail> = {
  ...
  'novel-breakdown': {
    requirement: ['接入真实文本分析 provider'],
    unavailable: ['小说拆解尚未接入真实分析 provider；Technical Preview 不返回示例分析冒充结果。（HTTP 501 · NOVEL_BREAKDOWN_NOT_IMPLEMENTED · truthfulPreviewRoutes fail-closed）'],
    contract: '无——下游 demo 生成器为被 gate 遮蔽的死代码，设计不提供假入口',
    landing: 'nav 05 → NovelBreakdownView（501 三态）',
  },
