...
| `replayInputs.candidates[].activation?` | 判别联合 `ActivationEvidence`（同 Q15 形状） | **T9 增补（#25）**：recomputationHash 覆盖 entries（含激活证据），converge 淘汰者的证据在 receipt entries 中无第二载体——缺此位则「任意时刻重放必得同哈希」（spec §3.1）在结构上不可满足 |
