...
export function mergeChannelsWithRRF(
  ...
  const entryMeta = new Map<string, { tier: RecallTier; primaryChannel: AssemblyChannel; content: string; pinned?: boolean; atomicOverride?: boolean }>()
  const channelParticipation = new Map<string, Set<AssemblyChannel>>()
  const exclusions: RecallExclusion[] = []
  ...
  for (const channel of channels) {
    ...
    sortedEntries.forEach((entry, idx) => {
      ...
      if (!channelParticipation.has(entry.id)) {
        channelParticipation.set(entry.id, new Set())
      }
      channelParticipation.get(entry.id)!.add(channel.channel)
  ...
  const candidates: RecalledCandidate[] = Array.from(rrfScores.entries()).map(([id, score]) => {
