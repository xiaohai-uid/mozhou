...
describe('内置模型断网冷启动（P0 硬断言）', () => {
  ...
  it('纯 CPU 单查询延迟冒烟：warm 后 5 次采样中位 <500ms（#24 AC①；T9 基准 18ms；#45 负载感知化二修）', async () => {
    // 负载感知的 CPU 冒烟而非精度断言（#45）：一修把单次 <50ms 改 5 采中位 <100ms，
    // 但全量 454 用例并行满载下中位仍实测冲到 108.5ms（探针尾刺 133ms），假红率 ~1/9——
    // 「3 连绿验收」遇 (8/9)³≈70% 假绿概率，判据不足。二修取 500ms：约等于
    // 基准（18-34ms，即 15-27× 余量）与满载观测最坏中位（~133ms）的 3.8× 双侧安全，
    // 对并行负载彻底不敏感；病态回归照样落网——热路径混入同步 IO 或模型改走网络
    // 加载时中位数达秒级，仍远超 500ms。此阈值此刻起是判定假红回归的黄金判据：
    // 满载连跑 ≥10 次若再出现非该断言失败，才是新问题。
