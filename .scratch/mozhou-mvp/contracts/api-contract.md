...
> ⚠️ 面区分：本文件描述 MVP `/api/v1`（带登录态、13 界面冻结参考），与现行应用的 `/api/*`（无版本前缀、本地无登录态）**不同源、不混用**。当前实现（含漫剧分镜面）的真源见 [`docs/contracts/storyboard-api.md`](../../../docs/contracts/storyboard-api.md)。
