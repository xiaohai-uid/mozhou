...
Spec 轴发现：409 时客户端采纳服务器 revision 会使下一次保存通过 CAS——「禁止静默覆盖」被推迟一拍而非阻止。已移除采纳逻辑：过期 revision 保留，后续保存持续 409，逼出刷新对账的人工路径；后台保存同样受阻（静默），用户重进章节时加载效果会以服务器行重置基准。字符串哨兵 `"conflict"` 经评估保留（结果对象改动的涟漪大于收益）。
